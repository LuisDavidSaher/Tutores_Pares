...
const Dashboard = () => {
  ...
  // CONEXIÓN AL BACKEND CON INTERCEPTOR JWT
  useEffect(() => {
    let montado = true;
    const cargarMetricas = async () => {
      try {
        ...
        // Se define únicamente la ruta relativa para el apiFetch
        const endpoint = `/api/dashboard/metricas?programa=${encodeURIComponent(paramPrograma)}`;
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-7xl mx-auto pb-10 min-h-screen">
      ...
      <div className="p-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm border-l-4 border-l-udc-primary hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Tutores Activos</p>
                <h3 className="text-4xl font-black text-udc-primary">{metricas.tutoresActivos}</h3>
              </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm border-l-4 border-l-udc-secondary hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Tutorados Activos</p>
                <h3 className="text-4xl font-black text-udc-primary">{metricas.tutoradosActivos}</h3>
              </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm border-l-4 border-l-blue-500 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-4xl font-black text-udc-primary">{metricas.asignaturas}</h3>
              </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm border-l-4 border-l-green-500 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-4xl font-black text-udc-primary">{metricas.reportesCompletados}</h3>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-gray-50 p-8 rounded-xl border border-gray-200 shadow-sm">
              <h4 className="text-lg font-bold text-udc-primary mb-4">Métricas de Riesgo Inicial (Tutorados)</h4>
              <div className="space-y-5">
                <div>
                  <div className="flex justify-between text-xs font-bold mb-1">
                    <span className="text-gray-600">Bajo Rendimiento (Riesgo Bajo)</span>
                    <span className="text-udc-primary">{metricas.riesgoBajo} estudiantes ({porcentajeBajo}%)</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                    <div className="bg-udc-primary h-2.5 rounded-full transition-all duration-1000" style={{ width: `${porcentajeBajo}%` }}></div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-2">
                   <h4 className="text-sm font-bold text-udc-primary">Top Programas (Tutores)</h4>
                </div>
                <ul className="space-y-3">
                  {metricas.topProgramas?.length > 0 ? metricas.topProgramas.slice(0, 5).map((prog, idx) => (
                    <li key={idx} className="flex justify-between items-center text-sm">
                      ...
                      <span className="bg-udc-primary text-white px-2 py-1 rounded text-xs font-bold">{prog.cantidad}</span>
                    </li>
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-2">
                   <h4 className="text-sm font-bold text-udc-primary">Top Asignaturas (Tutorías)</h4>
                </div>
                <ul className="space-y-3">
                  {metricas.topAsignaturas?.length > 0 ? metricas.topAsignaturas.slice(0, 5).map((asig, idx) => (
                    <li key={idx} className="flex justify-between items-center text-sm">
                      ...
                      <span className="bg-udc-secondary text-udc-primary px-2 py-1 rounded text-xs font-bold">{asig.cantidad}</span>
                    </li>
                  )) : <p className="text-xs text-gray-400 italic">No hay datos suficientes aún.</p>}
                </ul>
              </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <h3 className="text-lg font-bold text-udc-primary mb-5 border-b border-gray-100 pb-3 flex items-center">
              Guía Rápida
            </h3>
            <ul className="space-y-4">
              <li className="flex items-start">
                <span className="text-udc-secondary font-black mr-2">✓</span>
                <div>
                  <p className="text-xs font-bold text-udc-primary">Reporte Inicial</p>
              ...
              <li className="flex items-start">
                <span className="text-udc-secondary font-black mr-2">✓</span>
                <div>
                  <p className="text-xs font-bold text-udc-primary">Reporte Final</p>
            ...
            <div className="mt-8 bg-blue-50 p-4 rounded-lg border border-blue-100">
               ...
               <p className="text-lg font-black text-udc-primary">2026-I</p>
            </div>
          </div>
