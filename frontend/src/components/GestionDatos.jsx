...
const GestionDatos = () => {
  ...
  const cargarDatosMaestros = async () => {
    ...
    try {
      ...
      if (respuesta.ok) {
        ...
        console.error("Acceso denegado: El servidor rechazó la solicitud de lectura.");
        setDatos([]);
      }
    } catch (error) {
  ...
  const handleSubirArchivo = async (event) => {
    ...
    if (!archivo.name.endsWith('.xlsx') && !archivo.name.endsWith('.xls')) {
      alert('Validación fallida: El formato del archivo debe ser .xlsx o .xls');
    ...
    try {
      ...
      if (respuesta.ok) {
        alert('Carga masiva ejecutada correctamente en la base de datos.');
        cargarDatosMaestros();
      } else {
        alert('Carga denegada: Verifique el formato del documento o la validez de su sesión.');
      }
    } catch (error) {
  ...
  const renderIconoOrden = (campo) => {
    ...
    return <span className="text-udc-secondary ml-2 font-black text-xs">{ordenAsc ? '↑' : '↓'}</span>;
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-7xl mx-auto pb-10 min-h-screen">
      ...
      <div className="bg-udc-primary p-6 text-white border-b-4 border-udc-secondary flex justify-between items-center">
        <div>
        ...
        <div>
          ...
          <button onClick={() => fileInputRef.current.click()} disabled={subiendoArchivo} className={`bg-udc-secondary text-udc-primary px-5 py-2.5 rounded-lg font-bold transition-all text-sm shadow-sm flex items-center ${subiendoArchivo ? 'opacity-70 cursor-not-allowed' : 'hover:bg-yellow-500'}`}>
            {subiendoArchivo ? (
              <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-udc-primary mr-2"></div> Transmitiendo Datos...</>
            ) : (
      ...
      <div className="flex border-b border-gray-200 bg-gray-50">
        <button onClick={() => setPestaña('tutores')} className={`w-1/2 py-4 text-sm font-bold uppercase tracking-wider transition-all ${pestaña === 'tutores' ? 'border-b-4 border-udc-primary text-udc-primary bg-white' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}>
          Directorio: Tutores Pares
        </button>
        <button onClick={() => setPestaña('tutorados')} className={`w-1/2 py-4 text-sm font-bold uppercase tracking-wider transition-all ${pestaña === 'tutorados' ? 'border-b-4 border-udc-primary text-udc-primary bg-white' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}>
          Directorio: Tutorados
        </button>
      </div>
      ...
      <div className="p-8">
        ...
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="md:col-span-2">
            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Búsqueda Indexada</label>
            <input type="text" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Búsqueda por Identificación o Nombres..." className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-1 focus:ring-udc-primary" />
          </div>
          <div>
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm relative">
          {cargando && (
             <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] flex items-center justify-center z-10">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-udc-primary"></div>
             </div>
          )}
          <table className="w-full text-left border-collapse">
            <thead className="bg-udc-primary text-white text-xs uppercase font-semibold select-none">
              <tr>
                <th className="p-4 cursor-pointer hover:bg-gray-800 transition" onClick={() => handleOrden('documento')}><div className="flex items-center">Identificador {renderIconoOrden('documento')}</div></th>
                ...
                {pestaña === 'tutores' ? (
                  <><th className="p-4">Carga Académica</th><th className="p-4">Estado</th></>
                ) : (
                  <><th className="p-4 cursor-pointer hover:bg-gray-800 transition" onClick={() => handleOrden('riesgo')}><div className="flex items-center">Nivel de Riesgo {renderIconoOrden('riesgo')}</div></th><th className="p-4 cursor-pointer hover:bg-gray-800 transition" onClick={() => handleOrden('promedio')}><div className="flex items-center">Promedio {renderIconoOrden('promedio')}</div></th></>
                )}
              </tr>
            </thead>
            <tbody className="text-sm divide-y bg-white">
              {itemsActuales.length === 0 ? (
                <tr><td colSpan="7" className="p-8 text-center text-gray-500">Ausencia de registros en la base de datos.</td></tr>
              ) : (
                  ...
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    {pestaña === 'tutores' ? (
                      ...
                      <>
                        ...
                        <td className="p-4 font-bold text-udc-primary">{item.promedio}</td>
                      </>
                    )}
                  </tr>
                ))
